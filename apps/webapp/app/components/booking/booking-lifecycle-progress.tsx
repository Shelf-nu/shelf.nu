 ...
 * three proportional segments — Booked (gray) / Checked out (purple) / Returned
 * (green) — plus a legend with per-bucket counts. The headline label + count
 * switch by phase: while items remain Booked it reads "Check-out progress"
 * (count = checkedOut + returned); once nothing is Booked it reads "Check-in
 * progress" (count = returned). Backed by {@link calculateBookingLifecycleProgress}.
 *
...
export function BookingLifecycleProgress({ progress }: { progress: Progress }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div
        role="img"
        aria-label={`${bookedCount} booked, ${checkedOutCount} checked out, ${returnedCount} returned, out of ${totalUnits}`}
