...
export default function BulkPartialCheckoutDialog({
  ...
  // the booking-scoped asset record (which carries `status`, kit membership,
  // etc.). Post-pivot: assets live behind the BookingAsset pivot — dedup by
  // asset id since qty-tracked assets can appear on multiple pivot rows.
  const bookingAssetsMap = new Map(
    booking.bookingAssets.map((ba) => [ba.asset.id, ba.asset])
  );
