...
/**
 * Whether switching barcodes off has to wait because another live subscription
 ...
 */
async function barcodesStillCoveredElsewhere(
...
export async function handleBarcodeAddonWebhook({
  ...
  switch (eventType) {
    ...
    case "customer.subscription.updated": {
      ...
      if (
        !isActive &&
        (await barcodesStillCoveredElsewhere(subscription, organizationId))
      ) {
        break;
      }
      await db.organization.update({
    ...
    case "invoice.overdue": {
      if (await barcodesStillCoveredElsewhere(subscription, organizationId)) {
        break;
      }
      await db.organization.update({
        where: { id: organizationId },
