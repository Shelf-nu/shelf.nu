...
The companion mobile app in `apps/companion/` is not under AGPL-3.0. It has its [own license](https://github.com/Shelf-nu/shelf.nu/blob/main/apps/companion/LICENSE), which does not allow distribution. For that reason we can accept a pull request that changes files in `apps/companion/` only under a separate written agreement with Shelf Asset Management, Inc. Please open an issue before you start work there.
