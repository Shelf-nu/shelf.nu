...
type SetSearchParams = (setter: (prev: URLSearchParams) => URLSearchParams) => void;
...
export function checkValueInCookie(keys:string[], cookieSearchParams: URLSearchParams):boolean {
...
export function useSearchParamHasValue(...keys: string[]) {
  ...
  let hasValueInCookie = isAssetIndexPage && checkValueInCookie(keys, cookieSearchParams);
...
export function deleteKeysInSearchParams(keys:string[], setSearchParams:SetSearchParams){
  keys.forEach((key) => {
...
export function destoryCookieValues(organizationId:string, keys: string[], cookieSearchParams:URLSearchParams) {
  keys.forEach((key) => {
...
export function useClearValueFromParams(...keys: string[]) {
  ...
  const { isAssetIndexPage, organizationId, cookieSearchParams } = useAssetIndexMeta()
