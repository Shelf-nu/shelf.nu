...
/**
 * Generic hook for cookie destruction that accepts a custom cookie name
 * Use this when you need to work with cookies other than the default asset filter cookies
 *
 * @param {string} [defaultCookieName] - Optional default cookie name to use
 * @returns {Object} - An object containing the `destroyCookieValues` function
 */
export function useCookieDestroyGeneric(defaultCookieName?: string) {
  const cookieSearchParams = useAssetIndexCookieSearchParams();
  const currentOrganization = useCurrentOrganization();
  const { isAssetIndexPage, modeIsAdvanced } = useAssetIndexViewState();
  const isPageWithCookieFilters = useIsPageWithCookieFilters();
  ...
  /**
   * Function to destroy specific keys from cookies
   *
   * @param {string[]} keys - Array of keys (strings) to be removed from the cookies.
   * @param {string} [customCookieName] - Optional custom cookie name to use instead of the default
   */
  function _destroyCookieValues(keys: string[], customCookieName?: string) {
