...
| **Phase 4d — Auxiliary items umbrella**                                          | line 1520-1533            | Partially shipped. **Import qty + AssetModel round-trip — DONE 2026-06-16** for content import (create + update); see the "Import qty-tracked + AssetModel — DONE" section below. **Bulk-ops asset-type awareness — AUDITED 2026-06-24, already shipped** during Phase 4b Polish-2/3/4/5/6; see the "Phase 4d sub-item — bulk-ops asset-type awareness" audit-summary section below. Remaining sub-items still deferred: kit checkout/check-in qty polish, model grouping tool, backup-import AssetModel round-trip. |
...
#### Phase 4d sub-item — bulk-ops asset-type awareness (AUDITED 2026-06-24, ALREADY DONE)

The 2026-06-10 backlog listed "bulk operations awareness of asset types"
(PRD line 850) as a remaining Phase 4d sub-item. A systematic audit on
2026-06-24 confirmed the work shipped incrementally during Phase 4b
Polish-2/3/4/5/6 — the backlog table never received the receipt. No code
changes required.

**Service layer — every bulk-op that could mishandle QT is already
qt-aware or genuinely qt-na:**

| Bulk-op                                                                                                                 | Layer                          | QT handling                                                                                               |
| ----------------------------------------------------------------------------------------------------------------------- | ------------------------------ | --------------------------------------------------------------------------------------------------------- |
| `bulkCheckOutAssets` / `bulkCheckInAssets`                                                                              | `asset/service.server.ts`      | Filter `type !== QUANTITY_TRACKED` + throw 400 if all-QT                                                  |
| `bulkUpdateAssetLocation`                                                                                               | `asset/service.server.ts:5956` | Filter QT + throw 400 if all-QT                                                                           |
| `bulkAddToKit` (route)                                                                                                  | `assets.bulk-add-to-kit.ts:91` | Filter QT + throw 400 if all-QT                                                                           |
| `bulkRemoveAssetsFromKits`                                                                                              | `kit/service.server.ts:4807`   | Pivot-aware — reads `AssetKit.quantity` per row                                                           |
| `bulkDeleteKits`                                                                                                        | `kit/service.server.ts:2029`   | Reads `AssetKit.quantity` + `asset.type` for cascade notes                                                |
| `bulkAssignKitCustody`                                                                                                  | `kit/service.server.ts:2127`   | Reads `asset.type` + `asset.quantity` to inherit qty-tracked full quantity on kit-inherited Custody       |
| `bulkReleaseKitCustody`                                                                                                 | `kit/service.server.ts:2407`   | Captures `Custody.quantity` per kit-allocated row, cascades correctly (decrements by N, not by 1)         |
| `bulkUpdateKitLocation`                                                                                                 | `kit/service.server.ts:3221`   | Cascades new location to kit-driven `AssetLocation` rows with the kit's per-row `quantity` (Polish-4)     |
| `bulkDeleteAssets` / `bulkUpdateAssetCategory` / `bulkAssignAssetTags` / `bulkRemoveAssetTags` / `bulkMarkAvailability` | `asset/service.server.ts`      | qt-na — no quantity-dependent behavior; tags/category/availability/delete apply identically to both types |
| Audit add-assets, bulk-archive-bookings, bulk-cancel-bookings, custom-field bulk-(de)activate                           | various                        | qt-na — asset participates as a single object regardless of qty                                           |

**UX layer — all four QT-gating dialogs follow the canonical Polish-3
pattern:**

| Dialog                            | WarningBox | `isQuantityTracked` filter from `selectedBulkItemsAtom` | Copy                                                                                             |
| --------------------------------- | ---------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `bulk-assign-custody-dialog.tsx`  | line 48    | yes                                                     | "must be assigned custody individually with a specific quantity"                                 |
| `bulk-release-custody-dialog.tsx` | line 35    | yes                                                     | "must have custody released individually"                                                        |
| `bulk-location-update-dialog.tsx` | line 43    | yes (reference impl)                                    | "must have their placements managed individually with a per-location quantity"                   |
| `bulk-add-to-kit-dialog.tsx`      | line 60    | yes                                                     | "must be added to a kit individually with a specific quantity from the kit's manage-assets page" |

Each dialog renders the WarningBox above the action buttons summarising
the skipped QT count, and each paired service function throws a 400 with
the matching "All selected assets are quantity-tracked …" message when
the entire selection is qty-tracked — closing the empty-batch UX gap.

**Three formerly-unknown items resolved during the audit:**

- `audits.add-assets.ts` — qt-na. Audits are presence/condition scans;
  a QT asset participates as one object regardless of unit count.
- `bulkReleaseKitCustody` — qt-aware. Captures `Custody.quantity` per
  kit-allocated row before deletion, emits paired `CUSTODY_RELEASED`
  events with the correct per-asset quantity, renders unit counts in
  release notes via `formatUnitCount`.
- `bulkUpdateKitLocation` — qt-aware. Deletes kit-driven AssetLocation
  rows (`where: { assetKit: { kitId: { in: actualKitIds } } }`) and
  re-creates them at the new location with each row's `AssetKit.quantity`,
  preserving the kit-allocation slice across the move.

No follow-up work needed; the backlog row is closed.
